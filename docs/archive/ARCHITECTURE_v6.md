...
> **Agent-Pilot = 一个长在飞书 IM 里的双线 Agent —— 内层走 Claude Code 7 支柱（runtime / memory / domain），外层串联飞书 7 个 API，对外通过 MCP 暴露给所有外部 Agent。**
...
Agent-Pilot **已经有这 7 个机制的雏形**（不是从零）。这次架构升级是**把雏形提炼为统一框架**，而不是发明轮子。

### 7 支柱在 Agent-Pilot 中的现状 vs 目标
...
- [ ] step2 仓库拆分：agent-pilot_bcefghj/ 12 子目录
- [ ] step3 全局清理禁用文案
...
Agent-Pilot **不是 Claude Code 的克隆**，是**借鉴 Claude Code 工程哲学，落到飞书 IM 协同场景**。

| 维度 | Claude Code | Agent-Pilot |
|---|---|---|
...
| 为什么叫 Agent-Pilot 不是 LarkAssistant？ | "Assistant"是工具感，"Mentor"是关系感。我们要表达的是：双线服务都是为了"陪一个人在这家公司活下去"，关系感 > 工具感。 |

---

## 9 · 给下一个 AI 接手者的导读

如果你是接手 Agent-Pilot 的下一个 AI，按这个顺序读：
