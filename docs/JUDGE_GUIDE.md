...
应当看到 Agent 回复一条 **飞书交互卡片**（橙色标题「Agent-Pilot · 主动澄清」），包含：
- 提问列表（汇报对象、类型、时间等）
- **一键按钮**：「生成文档」「生成 PPT」「文档 + PPT 三件套」「跳过，直接开始」
- 用户点击任意按钮即可触发后续流程

➡️ 这验证了 PRD §5.3 的"信息不足时主动澄清"，且通过交互卡片让用户零输入即可回答。
...
- ✅ **在线状态面板**：Web Dashboard 顶栏显示当前所有在线端（Presence API）
- ✅ 新客户端 join 时，已有客户端收到 presence 广播通知
...
- ✅ **专业级 PPT**（多种布局模板 + 卡片/分栏/编号 + 渐变品牌色 + 演讲排练稿）
