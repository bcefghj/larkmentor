...
def test_weekly_no_llm_fallback(isolated_data):
    ...
    wm.events.append(WorkingEvent(ts=now + 1, kind="focus_end", payload={"duration_min": 25}))
    wm.events.append(WorkingEvent(ts=now + 2, kind="decision", payload={"level": "P0"}))
    wm.save()

    report = generate_weekly_report("ou_weekly", llm_chat=lambda p: "", publish=False)
...
def test_wrapped_returns_card(isolated_data):
    ...
    for _ in range(3):
        wm.events.append(WorkingEvent(ts=now, kind="focus_start", payload={}))
        wm.events.append(WorkingEvent(ts=now + 1, kind="focus_end", payload={"duration_min": 30}))
    wm.events.append(WorkingEvent(ts=now, kind="decision", payload={"level": "P0"}))
    wm.save()
