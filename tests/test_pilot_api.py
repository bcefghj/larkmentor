...
def test_pilot_launch_creates_plan(client):
    resp = client.post("/api/pilot/launch", json={"intent": "把本周讨论做成方案 + PPT", "open_id": "ou_api_test"})
...
def test_pilot_plan_detail_endpoint(client):
    resp = client.post("/api/pilot/launch", json={"intent": "帮我起草一个产品需求文档", "open_id": "ou_api_test"})
...
def test_pilot_list_filters_by_open_id(client):
    client.post("/api/pilot/launch", json={"intent": "demo a", "open_id": "ou_user_a"})
    client.post("/api/pilot/launch", json={"intent": "demo b", "open_id": "ou_user_b"})
...
def test_pilot_share_page_renders(client):
    ...
    launch = client.post("/api/pilot/launch", json={"intent": "分享页测试", "open_id": "ou_share_test"})
