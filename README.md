...
| 指令 | 效果 |
|------|------|
| `开启专注 30 分钟` | 进入专注模式，低优先级消息被拦截，30 分钟后自动提醒 |
| `开启专注` | 无限专注，直到手动结束 |
| `结束专注` | 退出专注模式，弹出 Recovery Card（显示被拦截的消息和自动起草的回复） |
| `我的状态` | 查看当前是否在专注，已专注多久，白名单有哪些人 |

### Mentor 表达带教

| 指令 | 效果 |
|------|------|
| `帮我看：<消息内容>` | 分析消息，给出 3 版不同语气的改写（默认对同事） |
| `帮我看老板：<消息内容>` | 同上，语气对应「对老板」 |
| `帮我看下属：<消息内容>` | 同上，语气对应「对下属」 |
| `任务确认：<任务描述>` | 评估任务模糊度，给出最应该问清楚的 2 个问题 |
| `写周报` | 从本周聊天/文档/任务自动生成 STAR 结构周报草稿 |
...
| 指令 | 效果 |
|------|------|
| `导入知识：<文本内容>` | 将文本存入个人知识库（后续 Mentor 会优先参考） |
...
| 指令 | 效果 |
|------|------|
| `白名单：张三` | 将"张三"加入白名单，其消息始终直接推送 |
| `删除我的数据` | 物理删除账号下所有数据（30 秒内完成，不可恢复） |
