...
def proactive_clarify(
    intent: str, *, chat_context: Optional[Iterable[Dict[str, Any]]] = None, min_chars: int = 10
) -> Optional[ClarifyRequest]:
...
def summarize_discussion(messages: List[Dict[str, Any]], *, max_chars: int = 4000) -> Dict[str, str]:
...
def recommend_next_steps(
    plan_summary: Dict[str, Any], *, user_open_id: str = "", max_suggestions: int = 3
) -> List[str]:
    ...
    try:
        ...
        mem = default_memory()
        recalled = mem.recall(f"next steps for user {user_open_id}", user_id=user_open_id, k=8) if user_open_id else []
    except Exception:
        recalled = []
