...
class AgentReport:
    ...
    def overall_ok(self) -> bool:
        return not self.safety_blocked and (
            self.quality_gates_total == 0 or self.quality_gates_passed >= self.quality_gates_total - 1
        )
...
class MultiAgentPipeline:
    ...
    # ── 主入口 ─────────────────────────────────────────────────────────────
    def run(self, task: Task, *, content: str = "", artifacts: Optional[List[Artifact]] = None) -> AgentReport:
    ...
    def _run_debater(self, task: Task, content: str, report: AgentReport) -> None:
        ts = AgentTranscript(
            agent="@debater", role="debate-pro-con", system_prompt="你将作为正反两方独立辩论 3 轮，最后给出收敛方案。"
        )
        t0 = time.time()
        if self.llm_chat:
            try:
    ...
    def _run_citation(self, task: Task, content: str, report: AgentReport) -> None:
        ts = AgentTranscript(
            agent="@citation", role="fact-checker", system_prompt="你独立给每条 claim 标 source，不做生成。"
        )
        t0 = time.time()
        ...
        if self.llm_chat and target:
            try:
                ...
                report.style_revisions = [
                    line.strip()
                    for line in ts.raw_response.splitlines()
        ...
        for priority in sorted(priority_groups.keys()):
            ...
            if len(group) == 1:
                ...
                with ThreadPoolExecutor(max_workers=min(max_parallel, len(group))) as pool:
                    futures = {pool.submit(self._run_single_agent, t, task): t for t in group}
