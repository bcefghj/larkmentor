...
def help_card() -> dict:
    ...
    card["elements"] = [
        _md(
            "**🚀 Agent 驱动任务（核心能力）**\n"
            "- 直接说：`帮我写个关于XX的文档` — AI 自动识别意图并执行\n"
            "- `/pilot 你的任务描述` — 显式启动 Agent 任务\n"
            "- 支持：文档生成、PPT/画布制作、数据分析、内容检索\n"
            "- Agent 会自动规划步骤，生成飞书文档/演示文稿"
        ),
        _divider(),
        _md(
            "**📄 文档 & 演示**\n"
            "- `帮我写个产品介绍文档` — 创建飞书 Docx 并填充内容\n"
            "- `做个关于XX的PPT` — 生成演示文稿大纲\n"
            "- `/pilot 整理会议纪要` — 复杂任务自动拆解执行"
        ),
        _divider(),
        _md(
            "**🛡️ 消息守护（Smart Shield）**\n"
...
def first_time_welcome_card() -> dict:
    ...
    card["elements"] = [
        _md(
            "你好！我是 **Agent-Pilot**，飞书里的 AI 办公协同伙伴。🚀\n\n"
            "**我能做什么？**\n"
            "- 🤖 **Agent 任务** — 直接说「帮我写个文档」或用 `/pilot` 启动复杂任务\n"
            "- 📄 **文档生成** — 自动创建飞书文档并生成内容\n"
            "- 📊 **PPT/画布** — 制作演示文稿和思维导图\n"
            "- 🛡️ **消息守护** — 专注模式，消息分级拦截\n"
            "- 💬 **表达引导** — 写作建议和措辞优化\n\n"
            "**快速开始：** 直接告诉我你想做什么！\n"
            "例如：`帮我写一份产品介绍文档` 或 `/pilot 做个季度复盘`"
        ),
        _divider(),
        _buttons(
            _btn("试一试：生成文档", {"action": "pilot_demo", "payload": "/pilot 帮我写一份产品介绍文档"}, "primary"),
            _btn("帮助", {"action": "help"}),
        ),
    ]
    return card
