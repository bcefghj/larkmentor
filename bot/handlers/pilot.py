...
def pilot_help_text() -> str:
    """Plain text help. For interactive UX, use ``help_card()`` instead."""
    return (
        "🚀 **Agent-Pilot v13 使用指南**\n\n"
        "**核心能力**（直接用自然语言说就行）：\n"
        "  • 写文档：`帮我写一份 AI Agent 发展趋势报告`\n"
        "  • 做 PPT：`做一份 8 页客户汇报 PPT`（生成真 .pptx）\n"
        "  • 画架构图：`画一张产品架构图`（生成 Mermaid + tldraw）\n"
        "  • 三件套：`产品方案 + 架构图 + 评审 PPT`（联动生成）\n\n"
        "**Agent 工作流**：\n"
        "  1️⃣ 三闸门意图识别（模糊则主动澄清）\n"
        "  2️⃣ DAG 任务规划（im → doc → canvas → slide → archive）\n"
        "  3️⃣ 4-Agent 工坊（调研 → 撰写 → 评审 → 演示设计）\n"
        "  4️⃣ 多端实时同步（飞书 / Web / Flutter）\n"
        "  5️⃣ 总结归档 + 分享链接\n\n"
        "**显式命令**：\n"
