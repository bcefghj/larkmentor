...
def send_text(receive_id: str, text: str, id_type: str = "open_id") -> bool:
    ...
    req = CreateMessageRequest.builder().receive_id_type(id_type).request_body(body).build()
...
def reply_text(message_id: str, text: str) -> bool:
    ...
    if not text:
        ...
        return False
    client = get_client()
    body = ReplyMessageRequestBody.builder().content(json.dumps({"text": text})).msg_type("text").build()
...
def reply_card(message_id: str, card: dict) -> bool:
    client = get_client()
    body = ReplyMessageRequestBody.builder().content(json.dumps(card)).msg_type("interactive").build()
    req = ReplyMessageRequest.builder().message_id(message_id).request_body(body).build()
    resp = client.im.v1.message.reply(req)
    if not resp.success():
